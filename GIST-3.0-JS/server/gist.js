	...
	let holyness = data.map((value, _index, _array) => {
		inp.forEach((value2, _index2, _array2) => {
			...
			if (value[value2.column_name] == null) {
				...
				/*
				replace 'Male' with 1 for the SVM model training
				*/
				value[value2.column_name] = 1;
			} else if (value[value2.column_name] == 8532 && value2.column_name == 'gender'){
				//value[value2.column_name] = "FEMALE";
				/*
				replace 'Female' with 2 for the SVM model training
				*/
				value[value2.column_name] = 2;
			}
		});
		delete value["p_id"];
...
function hyper_surface(input, patient_values) {
	...
	*/
	...
	/*
	 Sklearn.SVM.SVC from Python, used in GIST 3.0
	*/
	fs.writeFileSync('temp/features', JSON.stringify(features));		
	fs.writeFileSync('temp/labels', JSON.stringify(labels));		

	const execSync = require('child_process').execSync;
	console.log("training");
	const stdout = execSync('python py_svm.py ', { maxBuffer: 50000* 1024});
	//const stdout = execSync('python py_test.py ' +  JSON.stringify(features) +' '+ JSON.stringify(labels), { maxBuffer: 50000* 1024});
	const labels_pred = JSON.parse(stdout.toString());
...
function cal_crit(input, pat_row) {
	...
	let calculated_criterias = input.map(crit => {
		...
		if (crit.column_name[0] == "g") {
			...
			/*
			replace 'Male' with 1 and 'Female' with 2 for the SVM model training
			*/
